...
class Settings:
    # ---------- Paths ----------
    ROOT_DIR: str = _env("WEALL_ROOT_DIR", os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
    ...
    STATE_FILE: str = _env("WEALL_STATE_FILE", os.path.join(DATA_DIR, "executor_state.json"))
    CONTENT_INDEX_FILE: str = _env("WEALL_CONTENT_INDEX_FILE", os.path.join(DATA_DIR, "content_index.json"))
    ...
    # ---------- Security / CORS ----------
    CORS_ORIGINS: List[str] = [o.strip() for o in _env("WEALL_CORS_ORIGINS", "http://localhost:5173,http://127.0.0.1:5173,http://10.0.2.2:5173").split(",") if o.strip()]
    CORS_ALLOW_CREDENTIALS: bool = True

    # ---------- Content limits ----------
    MAX_UPLOAD_SIZE: int = int(_env("WEALL_MAX_UPLOAD_BYTES", str(25 * 1024 * 1024)))  # 25MB
    ...
    GOV_MIN_VOTERS_PCT: float = float(_env("WEALL_GOV_MIN_VOTERS_PCT", "0.10"))  # 10% of eligible voters
    GOV_PASS_PCT: float = float(_env("WEALL_GOV_PASS_PCT", "0.50"))              # simple majority of participating votes
    GOV_ELIGIBLE_TIER: int = int(_env("WEALL_GOV_ELIGIBLE_TIER", "2"))           # Tier-2+ can vote

    # ---------- Metrics ----------
    METRICS_ENABLED: bool = _env("WEALL_METRICS", "1") not in ("0","false","False")

    # ---------- Crypto provider ----------
    CRYPTO_PROVIDER: str = _env("WEALL_CRYPTO_PROVIDER", "pynacl")  # "pynacl" | "fallback_hmac"

    # ---------- Genesis / Bootstrapping ----------
    BOOTSTRAP_MODE: bool = _env("WEALL_BOOTSTRAP_MODE", "1") not in ("0","false","False")
    GENESIS_ADMIN_ENABLED: bool = _env("WEALL_GENESIS_ADMIN_ENABLED", "1") not in ("0","false","False")
