...
class PeerRegistry:
    ...
    def nonce_seen(self, nonce: str) -> bool:
        with self._lock:
            ...
            if nonce in self._announce_nonces:
                return True
    ...
    def new_secret(self) -> bytes:
        return secrets.token_bytes(32)  # AES-GCM key

    def set_room_secret(self, token: str, key: bytes):
        self._room_secrets[token] = key

    def get_room_secret(self, token: str) -> Optional[bytes]:
        return self._room_secrets.get(token)

    def del_room_secret(self, token: str):
        self._room_secrets.pop(token, None)
