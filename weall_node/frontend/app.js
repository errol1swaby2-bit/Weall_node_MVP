...
// === toast helpers (auto-dismiss) ===
window.toast = window.toast || function toast(msg, type){
...
window.toastWarn = (m)=> toast(m,'warn');
window.toastErr = (m)=> toast(m,'err');


// === production helpers ===
window.getJSON = window.getJSON || (async function getJSON(url){
...
window.toast = window.toast || function toast(msg, type){
  ...
  t.className = type || 'info'; t.textContent = msg;
};
window.withBusy = window.withBusy || (async function withBusy(btn, fn){
...
// Ensure top CTA always navigates
(function(){
  var el = document.getElementById('ctaSignupTop');
  if (!el) return;
  el.addEventListener('click', function(ev){
    // best-effort hard navigation
