...
POH_REQUIREMENTS = {
    "propose": 3, "vote": 2, "post": 2, "comment": 2, "dispute": 3, "juror": 3
}

def safe_int(prompt):
    try: return int(input(prompt))
    except: print("Invalid number"); return None

def run_cli():
    ex = WeAllExecutor(dsl_file="weall_dsl_v0.5.yaml", poh_requirements=POH_REQUIREMENTS)
    ...
    while True:
        ...
        if cmd == "exit":
            ...
            u = input("User ID: ").strip(); poh = safe_int("PoH Level (1-3): ")
            if poh is None: continue
            print(ex.register_user(u, poh_level=poh))

        elif cmd == "post":
            u = input("User ID: ").strip()
            c = input("Post content: "); tags = input("Tags (csv, optional): ").strip()
            tags = [t.strip() for t in tags.split(",")] if tags else None
            print(ex.create_post(u,c,tags))

        elif cmd == "comment":
            u = input("User ID: ").strip(); pid = safe_int("Post ID: ")
            if pid is None: continue
            c = input("Comment content: "); tags = input("Tags (csv, optional): ").strip()
            tags = [t.strip() for t in tags.split(",")] if tags else None
            print(ex.create_comment(u,pid,c,tags))

        elif cmd == "balance":
            u = input("User ID: ").strip(); print({"balance": ex.ledger.balance(u)})

        elif cmd == "transfer":
            a = input("From user: ").strip(); b = input("To user: ").strip(); amt = safe_int("Amount: ")
        ...
        # --- Governance ---
            ...
            u = input("User ID: ").strip(); pid = safe_int("Proposal ID: ")
            if pid is None: continue
            ...
            u = input("Enactor (Tier-3) User ID: ").strip(); pid = safe_int("Proposal ID: ")
            if pid is None: continue
            print(ex.try_enact_proposal(u,pid))

        elif cmd == "list_proposals":
            props = [ex.state["proposals"][k] for k in sorted(ex.state["proposals"].keys())]
            for p in props:
                print(f"[{p['id']}] {p['type']} {p['module']} — {p['status']} votes={len(p['votes'])}")
            if not props: print("No proposals.")
        ...
        # --- Messaging ---
            ...
            a = input("From user: ").strip(); b = input("To user: ").strip(); msg = input("Message: ").strip()
            print(ex.send_message(a,b,msg))

        elif cmd == "read_messages":
            u = input("User ID: ").strip(); 
            for m in ex.read_messages(u): print(f"From {m['from']} | {m['ts']}: {m['text']}")
