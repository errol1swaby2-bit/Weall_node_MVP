...
def run_cli():
    ...
    while True:
        cmd = input(
            ...
            "poh_request/poh_verify/poh_status/poh_tier2/poh_tier3/"
            "exit): "
        ).strip().lower()
        ...
        # ------------------------
        # API-backed PoH commands
        # ------------------------
        elif cmd == "poh_request":
            user = input("User ID: ")
            email = input("Email: ")
            r = requests.post(f"{API_URL}/poh/request-tier1", json={"user": user, "email": email})
            print(r.json())

        elif cmd == "poh_verify":
            user = input("User ID: ")
            code = input("Verification code: ")
            r = requests.post(f"{API_URL}/poh/verify-tier1", json={"user": user, "code": code})
            print(r.json())

        elif cmd == "poh_status":
            user = input("User ID: ")
            r = requests.get(f"{API_URL}/poh/status/{user}")
            print(r.json())

        elif cmd == "poh_tier2":
            user = input("User ID: ")
            evidence = input("Evidence CID: ")
            r = requests.post(
                f"{API_URL}/poh/tier2",
                json={"user": user, "evidence": evidence},
            ...
            user = input("User ID: ")
            video = input("Video proof CID: ")
            r = requests.post(
                f"{API_URL}/poh/tier3",
                json={"user": user, "video_proof": video},
                headers={"X-User-ID": user}
            )
            print(r.json())
